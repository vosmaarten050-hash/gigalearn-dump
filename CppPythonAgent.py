...
class BaseCPPAgent(BaseIndependentAgent):
    ...
    def run_independently(self, terminate_request_event):
            ...
            if sock is None:
                try:
                   ...
                   sock.sendall(message.encode("ascii"))
                    # optionally set non-blocking or timeout for periodic health checks
                   sock.settimeout(1.0)
                   self.logger.info("Connected to C++ helper and sent add. Keeping socket open.")
        ...
        # Poll the socket to detect server-side close/reset.
        try:
            data = sock.recv(1)   # small recv with timeout detects remote close
            if data == b'':      # remote closed
                self.logger.info("Helper closed connection; will try to reconnect")
                try: sock.close()
                except: pass
                sock = None
        except socket.timeout:
            # everything ok � remote still alive
            pass
        except Exception as e:
            self.logger.warn("Socket error, reconnecting: {}".format(e))
            try: sock.close()
            except: pass
            sock = None
        ...
        # Termination: send remove and close socket if connected
        try:
            if sock:
                remove_msg = f"remove\n{self.index}"
                sock.sendall(remove_msg.encode("ascii"))
                sock.close()
        except Exception:
            pass
